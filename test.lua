...
-- ==============================================================
-- FUNGSI COLLECT (Radar Infinite & Prioritas Top-Down)
-- ==============================================================
local cachedPrompts = {}  
...
local function updatePromptCache()
    ...
    task.spawn(function()
        ...
        for _, obj in pairs(Workspace:GetDescendants()) do
            ...
            if obj:IsA("ProximityPrompt") and obj.Parent and obj.Parent:IsA("BasePart") then
                local itemName = DapatkanNamaItem(obj)
                if itemName then
                    table.insert(newCache, {prompt = obj, part = obj.Parent, itemName = itemName})
                end
            end
        end
...
local function JalankanAutoCollectGlobal()
    ...
    -- FASE 1: SEDOT SEMUA ITEM DI DEKAT KARAKTER (Radius < 12)
    local microDelay = 0 -- Variabel pengatur antrean jaringan
    
    for obj, entry in pairs(cachedPrompts) do
        if obj and obj.Parent and not BlacklistedItems[obj] then
            local namaItem = entry.itemName
            ...
            if IsItemMatchForCollect(namaItem, rarityStr) then
                local delta = entry.part.Position - charPos
                ...
                if distToChar < 12 then
                    ...
                    BlacklistedItems[obj] = true
                    
                    -- [PERBAIKAN] Tembak prompt dengan jeda 0.03 detik per item
                    task.delay(microDelay, function()
                        if obj and obj.Parent then
                            pcall(function() fireproximityprompt(obj) end)
                        end
                        -- Hapus dari blacklist setelah dieksekusi agar aman
                        task.delay(0.3, function() if obj then BlacklistedItems[obj] = nil end end)
                    end)
                    
                    -- Tambahkan waktu 0.03 detik untuk item berikutnya
                    microDelay = microDelay + 0.03 
                else
                    -- Jika jauh, catat jaraknya untuk dieksekusi di Fase 2
                    if distToChar < jarakTeleportTerdekat then
                        jarakTeleportTerdekat = distToChar
                        targetTeleport = entry
                    end
                end
            end
        end
    ...
    -- FASE 2: JIKA TIDAK ADA YANG DEKAT, BARU TELEPORT KE ITEM TERDEKAT
